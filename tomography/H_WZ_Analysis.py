...
def main():
    ...
    plot_2d_2(gmarray_asym, cm.RdYlBu, 'H_wz_asym')
    plot_2d_2(gmarray_sym, cm.RdYlBu, 'H_wz_sym')
    print(f"\n\nDone in {time.time() - t_start}\n")

def run_zz_tomography():
    import ROOT
    from H_WZ import QuantumTomographyH_WZ
    ...
    analysis = QuantumTomographyH_WZ(tree)
...
def plot_3d_smooth(array):
    ...
    plt.xlabel('$\Lambda_2$')
    plt.gca().xaxis.tick_bottom()
    plt.title(r'$\rho(pp \rightarrow WZ)$')
    plt.show()
...
def plot_2d_2(array, colour, name):
    ...
    plt.xticks()
    plt.title(r'$\rho(pp \rightarrow WZ)$', fontsize=18, y = 1.04)
