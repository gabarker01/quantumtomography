...
def main():
    ...
    plot_2d_2(gmarray_sym, cm.RdYlBu, 'H_zz_sym')
    plot_2d_2(gmarray_asym, cm.RdYlBu, 'H_zz_asym')
    print(f"\n\nDone in {time.time() - t_start}\n")

def run_zz_tomography():
    import ROOT
    from H_ZZ2 import QuantumTomographyH_ZZ
    ...
    analysis = QuantumTomographyH_ZZ(tree)
    P2, P1, cov, cov_sym = analysis.tree_analysis()
    ...
    gmarray_sym = gmarray_sym / 2
    print(gmarray_sym) #EXTRA FACTOR OF 0.5 INCLUDED

    # Calculate asym matrix
    for i in range(8):
        gmarray_asym[0][i+1] = 0.5 * P2[i] #Z
...
def cmb_entanglement_test(gmarray):
    ...
    print("c_mb_squared:", c_mb_squared)
...
def plot_2d_2(array, colour, name):
    ...
    plt.ylabel('$Z$' + " GM Index")
    plt.xlabel('$Z$' + " GM Index")
    plt.xticks()
    plt.title(r'$\rho(H \rightarrow ZZ)$', fontsize=18, y = 1.04)
